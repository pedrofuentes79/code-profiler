...
def test_profiler_chooses_correct_snippet_with_geometries():
    ...
    results_apply = profiler.results_from(apply, gdf)
    results_geopandas = profiler.results_from(geopandas, gdf)
...
def test_profiler_chooses_correct_snippet_vectorized_operation():
    ...
    assert profiler.is_faster_than(efficient_sum, apply_sum, df_numeric)
...
def test_profiler_knows_which_snippet_uses_less_memory():
    ...
    assert profiler.is_more_memory_efficient_than(memory_efficient, memory_inefficient, df_numeric)
